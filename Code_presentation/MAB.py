...
# Ajouter le répertoire de travail courant au chemin de recherche des modules
...
class BanditEnvironment:
    ...
    def get_reward(self):
        # on génère un nombre aléatoire à chaque appel de la fonction
        ...
        V2V_simulation = V2V(1000 + r * d, 0.05 + td*r)             # PARAMETRES A CHANGER + mettre de l'aleatoire pour varier les résultats
        ...
        # V2I
        V2I_simulation = V2I(1000 + r*d, 0.5 + tl*r)              # PARAMETRES A CHANGER + mettre de l'aleatoire pour varier les résultats
...
def epsilon_greedy(env, k, T):
    ...
    for t in range(T):
        ...
        if regret == 0:
            bons_choix_EG.append(2)
        else :
            bons_choix_EG.append(0)
        # on enregistre les choix bras de la machine
        if arm == 0:
            env.V2V_Epsilon += 1
        else:
...
def ucb(env, k, T):
    ...
    for t in range(T):
        if t < k:
            ...
            if regret == 0:
                bons_choix_UCB.append(2)
            else :
                bons_choix_UCB.append(0)
...
def run_bandit(env, k, T):
    ...
    eps_regrets, eps_bons_choix = epsilon_greedy(env, k, T)
    ucb_regrets, ucb_bons_choix = ucb(env, k, T)
    print("ok")
    return eps_regrets, eps_bons_choix, ucb_regrets, ucb_bons_choix
    ...
    # # Regret par itération
    # plt.figure()
    # plt.title(f"T = {T} l'horizon (nb itération) -> évolution des bons choix")
    # plt.plot(np.cumsum(eps_bons_choix), label="Epsilon-Greedy")
    # plt.plot(np.cumsum(ucb_bons_choix), label="UCB")
    # plt.xlabel("Time")
    # plt.ylabel("Evolution du nombre de bons choix ")
    # plt.legend()
    # plt.show()
