...
class BanditEnvironment:
    ...
    def get_reward(self):
        ...
        distance = np.random.randint(1100, 1400) # loi uniforme entre 1000 et 3000 qui retourne un entier
...
def ucb(env, k, T):
    ...
    for t in range(T):
        if t < k:
            ...
            ucb_values = [est_means[i] + np.sqrt(2*np.log(t) / n[i]) for i in range(k)]  # Calculer les valeurs UCB pour chaque brass
